...
def promote_staging_to_production():
    ...
    try:
        ...
        raise RuntimeError(f"No active version found with alias '@staging' for model '{MODEL_NAME}'.")
    ...
    try:
        ...
        logger.info(f"Previous production model v{prod_version.version} demoted to '@archived'.")
    except Exception:
    ...
    logger.info(f"Successfully promoted model '{MODEL_NAME}' v{staging_version.version} to '@production'.")
